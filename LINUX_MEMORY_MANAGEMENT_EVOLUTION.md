...
> 📚 **首次阅读？** 建议先查看 [文档导读指南](READING_GUIDE.md#-内存管理机制)，了解内存管理文档的完整学习路径。
...
└─────────────────────────────────────────────────────────────────┘
                          ↓
         ① 第一次替换页表：CR3 从 pgtable → early_top_pgt
            （startup_64，head_64.S，进入主内核时）
                          ↓
┌─────────────────────────────────────────────────────────────────┐
│ 主内核 x86_64_start_kernel (arch/x86/kernel/head64.c:219)      │
│ （此时 CR3 = early_top_pgt）                                     │
├─────────────────────────────────────────────────────────────────┤
...
│ ② 第二次替换页表：本步完成后 CR3 从 early_top_pgt → swapper_pg_dir │
│                                                                  │
│ 状态：✅ 分页已启用（CR3 = swapper_pg_dir）                        │
│       ✅ memblock 已建立（可用于早期内存分配）                     │
│       ✅ 完整内存映射已建立                                       │
│       ⚠️  只能用 memblock 分配，buddy/slab 还未初始化             │
│       ❌ 还不能创建进程（进程调度器未初始化）                       │
...
“完全替换之前创建的页表”有两次，对应前文时间线图中的位置如下。

**1. 第一次替换：pgtable 被换掉**

- **谁被换掉**：压缩内核建的 **pgtable**（identity mapping，约 4GB）。
- **发生在哪**：主内核入口（**startup_64**，在 `head_64.S`），把 CR3 从 pgtable 改成 **early_top_pgt**。
- **在时间线图里的位置**：发生在**第 1 个框（压缩内核）之后、第 2 个框（x86_64_start_kernel）之前**，也就是从压缩内核跳到主内核、执行 startup_64 的那一段。图里没有单独画成一步，但时间上在“第 2 步”之前。
- **结论**：第一次“完全替换之前创建的页表” = 进入主内核时用 **early_top_pgt** 替换 **pgtable**，对应“第 1 步之后、第 2 步之前”的切换。

**2. 第二次替换：early_top_pgt 被换掉**

- **谁被换掉**：主内核早期的 **early_top_pgt**（双映射、过渡用）。
- **发生在哪**：**setup_arch()** 里调用 **init_mem_mapping()** 建好完整 direct mapping 后，内核会改用 **swapper_pg_dir**（即把 CR3 换成 swapper_pg_dir）。
- **在时间线图里的位置**：就是**第 3 个框**里的这一步：
  ```
  │ ✨ init_mem_mapping()            (setup.c:1119)                 │
  │    └─ 为所有 RAM 建立直接映射页表                                 │
  ```
  文档里“页表演化：pgtable → early_top_pgt → swapper_pg_dir”和“主内核运行 | swapper_pg_dir”说的就是：在 setup_arch()（init_mem_mapping()）之后，CR3 指向 swapper_pg_dir，early_top_pgt 不再被使用。
- **结论**：第二次“完全替换之前创建的页表” = **第 3 步 setup_arch()** 里的 **init_mem_mapping()** 完成后，用 **swapper_pg_dir** 完全替换 **early_top_pgt**。

**直接回答“图里哪一步会完全替换之前创建的页表”**：**第 3 步**（start_kernel() → setup_arch()）——这里通过 init_mem_mapping() 建立完整直接映射并切换到 swapper_pg_dir，完全替换掉之前用的 early_top_pgt。另外，pgtable 是在进入主内核时（第 1 步之后、第 2 步之前）被 early_top_pgt 替换的，图里没有单独标成一步。
