...
setup_keyboard_handler:
    ...
    ; - 前2字节：偏移地址（offset）
    ; - 后2字节：段地址（segment）
    ; 所以中断号N的向量地址 = 0x0000 + N * 4
    ; 例如：INT 09h = 0x0000 + 0x09 * 4 = 0x0024
...
- **为什么要乘以4？**
  ...
  **中断向量表（IVT）的结构：**
  ...
  在x86实模式下，中断向量表（IVT）位于 `0x0000:0000`，每个中断向量占**4字节**：
  ...
  **地址计算：**
  ...
  - **中断向量表起始地址**：`0x0000:0000` = 物理地址 `0x00000`
  - **每个中断向量占4字节**
  - **中断号N的向量地址** = `0x00000 + N × 4`
  
  **示例：**
  
  ```
  INT 00h：地址 = 0x00000 + 0x00 × 4 = 0x00000
  INT 01h：地址 = 0x00000 + 0x01 × 4 = 0x00004
  INT 02h：地址 = 0x00000 + 0x02 × 4 = 0x00008
  ...
  INT 09h：地址 = 0x00000 + 0x09 × 4 = 0x00024  ← 这就是为什么乘以4
  INT 10h：地址 = 0x00000 + 0x10 × 4 = 0x00040
  ...
  INT FFh：地址 = 0x00000 + 0xFF × 4 = 0x0003FC
  ```
  ...
  **内存布局示意：**
  ...
  ```
  物理地址    内容
  0x00000    [INT 00h: 偏移地址（低2字节）]
  0x00002    [INT 00h: 段地址（高2字节）]
  0x00004    [INT 01h: 偏移地址（低2字节）]
  0x00006    [INT 01h: 段地址（高2字节）]
  ...
  0x00024    [INT 09h: 偏移地址（低2字节）]  ← keyboard_handler的偏移
  0x00026    [INT 09h: 段地址（高2字节）]    ← keyboard_handler的段地址
  ...
  0x003FC    [INT FFh: 偏移地址（低2字节）]
  0x003FE    [INT FFh: 段地址（高2字节）]
  ```
  
  **代码中的使用：**
  
  ```asm
  mov bx, 0x09 * 4      ; bx = 0x24（INT 09h的向量地址）
  mov ax, [es:bx]       ; 读取偏移地址（0x00024处的2字节）
  mov ax, [es:bx+2]     ; 读取段地址（0x00026处的2字节）
  ```
