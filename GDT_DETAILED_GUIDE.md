...
│                                                              │
│ 5. 拆分线性地址，准备查询页表                                │
│    线性地址 0x123456 (假设使用 2MB 大页)                     │
│    = 0000_0000_0000_0001_0010_0011_0100_0101_0110 (二进制)  │
│                                                              │
│    PML4 索引 = bits [47:39] = 0                             │
│    PDPT 索引 = bits [38:30] = 0                             │
│    PD 索引   = bits [29:21] = 0 (目录项0)                   │
│    页内偏移  = bits [20:0]  = 0x123456                      │
│                                                              │
│ 6. 页表遍历（假设使用 2MB 大页）                             │
│                                                              │
│    a. 第一级：读取 PML4[0]                                   │
│       PML4 表物理地址 = CR3 = 0x2000                        │
│       PML4[0] 物理地址 = 0x2000 + (0 × 8) = 0x2000          │
│                                                              │
│       从内存[0x2000] 读取 8 字节：                           │
│       PML4[0] = 0x0000000000003003                          │
│                                                              │
│       页表项结构（64位）：                                   │
...
│                                                              │
│       提取 PDPT 物理地址：                                   │
│       PDPT 基址 = PML4[0] & 0xFFFFFFFFF000                  │
│                = 0x0000000000003003 & 0xFFFFFFFFF000        │
│                = 0x0000000000003000                         │
│                                                              │
│    b. 第二级：读取 PDPT[0]                                   │
│       PDPT 表物理地址 = 0x3000                              │
│       PDPT[0] 物理地址 = 0x3000 + (0 × 8) = 0x3000          │
│                                                              │
│       从内存[0x3000] 读取 8 字节：                           │
│       PDPT[0] = 0x0000000000004003                          │
│                                                              │
│       页表项结构：                                           │
...
│                                                              │
│       提取 PD 物理地址：                                     │
│       PD 基址 = PDPT[0] & 0xFFFFFFFFF000 = 0x4000           │
│                                                              │
│    c. 第三级：读取 PD[0]（2MB 大页）                         │
│       PD 表物理地址 = 0x4000                                │
│       PD[0] 物理地址 = 0x4000 + (0 × 8) = 0x4000            │
│                                                              │
│       从内存[0x4000] 读取 8 字节：                           │
...
│       │                                                  │  │
│       │ 二进制：                                         │  │
│       │ 0000_0000_0000_0000_0000_0000_0000_0001_1000_0011│ │
│       │                                                  │  │
│       │ 字段分解：                                       │  │
│       │ ┌─ Bits [63:52]: 保留 = 0                      │  │
...
│                                                              │
│       提取物理页框基址（2MB 对齐）：                         │
│       物理页框基址 = PD[0] & 0xFFFFFFFE00000                │
│                   = 0x0000000000000183 & 0xFFFFFFFE00000   │
│                   = 0x0000000000000000                     │
│                                                              │
│       解释：                                                 │
│       - 0xFFFFFFFE00000 掩码保留 bits [51:21]               │
│       - 2MB = 0x200000，需要 21 位对齐                      │
│       - Bits [20:0] 被清零，得到 2MB 对齐的基址             │
│                                                              │
│    ┌─────────────────────────────────────────────────────┐  │
│    │ 📌 页表查找过程总结                                 │  │
│    ├─────────────────────────────────────────────────────┤  │
...
│    │                                                      │  │
│    │ 具体应用：                                           │  │
│    │ • PML4[0] 地址 = CR3 + (0 × 8) = 0x2000            │  │
│    │ • PDPT 基址    = PML4[0] & 0xFFFFFFFFF000 = 0x3000 │  │
│    │ • PDPT[0] 地址 = 0x3000 + (0 × 8) = 0x3000         │  │
│    │ • PD 基址      = PDPT[0] & 0xFFFFFFFFF000 = 0x4000 │  │
│    │ • PD[0] 地址   = 0x4000 + (0 × 8) = 0x4000         │  │
│    │                                                      │  │
│    │ 关键区别（与 GDT 对比）：                            │  │
│    │                                                      │  │
│    │ GDT 查找：                                           │  │
...
│    │                                                      │  │
│    │ 为什么 0x00000183 → 0x00000000？                    │  │
│    │ • 完整值：0x0000000000000183                        │  │
│    │ • 掩码：  0xFFFFFFFE00000 (保留 bits [51:21])      │  │
│    │ • 结果：  0x0000000000000000                        │  │
...
2. **Paging 的输入**：
   - Paging 接收的是**线性地址**（0x123456），不是段基址
