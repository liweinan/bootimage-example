...
**完整列举（向量 0-31 中的所有 Trap 类型）**：

> 📖 **参考**：Intel SDM Volume 3A, Table 6-1: Protected-Mode Exceptions and Interrupts

| 向量 | 名称 | 说明 | 用途 | Intel SDM Type |
|------|------|------|------|---------------|
| **#DB (1)** | Debug | 调试异常（部分情况） | 单步执行、硬件断点 | **Fault/Trap** ⚠️ |
| **#BP (3)** | Breakpoint | 断点异常 | 调试器设置断点（`INT 3`） | **Trap** |
| **#OF (4)** | Overflow | 溢出异常 | 检测算术溢出（`INTO` 指令，32位） | **Trap** |

**关键发现**：
- ✅ 以上是向量 0-31 中**全部的 Trap 类型异常**（仅 3 个）
- ⚠️ #DB 特殊：既可以是 **Fault**（指令断点），也可以是 **Trap**（单步执行、数据断点）
- 对比：向量 0-31 中有 **13+ 个 Fault 类型异常**（#DE, #BR, #UD, #NM, #TS, #NP, #SS, #GP, #PF, #MF, #AC, #XM, #VE 等）

**为什么 Trap 异常这么少？**
- **Trap 的定义**：在指令执行**完成后**报告，保存的 RIP 指向**下一条指令**
- **主要用途**：调试和监控（断点、单步、溢出检测）
- **大多数异常属于 Fault**：因为大部分错误需要在**执行失败时**报告，允许系统修复后重新执行
