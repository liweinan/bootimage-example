   ...
   flowchart TB
       subgraph DRAM["物理内存（DRAM芯片）"]
           ...
           subgraph Above1MB["1MB-4GB RAM区域"]
               ...
               RelocatorMovers["Relocator movers_chunk（动态生成，非 relocator32.S）<br/>重要：movers_chunk 与 0x100000 (1MB) 是两块不同内存——movers_chunk 在 1MB 之上（如 16MB+）单独分配，0x100000 仅是复制写入目标，复制不会覆盖 movers_chunk。<br/>内容：preamble + forward/backward + jumper（relocator_common_c.c、relocator_asm.S 模板）。<br/>职责：从临时缓冲区 (src, 16MB+) 复制到 0x100000 (1MB)；执行完后 jumper 跳转安全区 relocator32 入口。<br/>生命周期：grub_relocator_prepare_relocs() 分配并写入 → boot 时执行一次"]
