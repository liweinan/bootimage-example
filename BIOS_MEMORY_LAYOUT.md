   ...
   flowchart TB
       subgraph DRAM["物理内存（DRAM芯片）"]
           ...
           subgraph Above1MB["1MB-4GB RAM区域"]
               ...
               RelocatorMovers["Relocator movers_chunk（动态生成，非 relocator32.S）<br/>内存位置：16MB+ 或 modend 以上，GRUB 空闲内存池动态分配；与 0x100000 (1MB) 为不同块。<br/>涉及文件与函数：relocator.c 分配；relocator_common_c.c 的 grub_cpu_relocator_preamble、grub_cpu_relocator_forward/backward（拷贝模板）、grub_cpu_relocator_jumper；relocator_asm.S 提供 forward/backward 模板。<br/>执行：preamble → forward/backward（src→0x100000 (1MB)）→ jumper 跳安全区 relocator32 入口。<br/>生命周期：grub_relocator_prepare_relocs() 分配并写入 → boot 时执行一次"]
