...
> 📚 **首次阅读本文档？** 建议先查看 [文档导读指南](READING_GUIDE.md)，了解完整的学习路径和文档体系。
...
> **内存管理文档体系导读**
>
> 本节包含 **14 个内存管理文档**，按学习路径组织为 5 个层次：
>
> 1. **理论基础** (1个文档) - 为什么需要虚拟内存
> 2. **内核空间内存管理** (4个文档) - 启动过程的内存管理演化（E820、GDT、页表）
> 3. **物理内存分配** (2个文档) - Buddy 系统和 Slab 分配器
> 4. **运行时内存管理** (2个文档) - Page Fault 和用户空间内存
> 5. **补充技术细节** (5个文档，可选) - BIOS/GRUB 如何构建和传递内存信息
>
> **推荐阅读顺序**：
> - **快速入门**：1 → 2 → 3 → 4（核心文档，理解主线）
> - **深入研究**：1 → 2 → 5 → 3 → 4（完整路径，理解底层细节）
> - **问题驱动**：直接查看相关文档，按需交叉引用
