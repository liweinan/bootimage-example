...
**核心问题**：为什么现代系统有 APIC 还需要初始化 PIC？

虽然现代 x86 系统都配备了 APIC，但 Linux 内核仍然会初始化传统的 8259A PIC。这背后有六大技术原因：

1. **硬件兼容性**：部分老旧主板的 Timer IRQ 只连接到 PIC
2. **ExtINT 模式**：PIC 通过 ExtINT 级联到 APIC LINT0
3. **代码依赖**：数百处代码依赖 `nr_legacy_irqs()` 和 `has_legacy_pic()`
4. **抽象层设计**：`legacy_pic` 支持物理机和虚拟机统一接口
5. **Virtual Wire Mode**：SMP 启动早期 APIC 从 PIC 接收中断
6. **初始化后 Mask**：PIC 初始化 ≠ PIC 被使用（IMR = 0xFF）
...
{
    x86_init.irqs.pre_vector_init();  // ⭐ 初始化 PIC（或 noop）
...
// Legacy PIC 抽象层
struct legacy_pic *legacy_pic = &default_legacy_pic;  // 物理机
legacy_pic = &null_legacy_pic;  // 虚拟化环境（ACPI/Jailhouse）
```

**共存策略总结**：

| 场景 | PIC 初始化 | PIC 使用 | APIC 状态 |
|------|-----------|---------|----------|
| 老旧物理机（无 IO-APIC） | ✅ 真实初始化 | ✅ 使用 | ❌ 未启用 |
| 老旧物理机（Timer 连 PIC） | ✅ 真实初始化 | ⚠️ 部分使用 | ✅ 启用 |
| 现代物理机（完整 APIC） | ✅ 真实初始化 | ❌ 全部 mask | ✅ 启用 |
| 虚拟化环境 | ❌ noop | ❌ 不使用 | ✅ 虚拟 APIC |

**详细说明**：参见独立文档 → **[PIC 与 APIC 的共存详解](PIC_APIC_COEXISTENCE.md)**
