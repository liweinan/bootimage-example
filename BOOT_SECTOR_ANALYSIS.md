...
**输出示例（hexdump，默认格式）：**
```
0000000 03b8 cd00 be10 7c15 0eb4 84ac 74c0 cd04
0000010 eb10 ebf7 48fe 6c65 6f6c 6620 6f72 206d
0000020 6f42 746f 5320 6365 6f74 2172 0000 0000
...
**说明：**
- 左侧是文件偏移量（十六进制）
- 中间是每 16 字节的十六进制内容（小端序，每 2 字节一组）
- `*` 表示重复的零字节（从 0x0030 到 0x01F0 都是 0）
- 最后一行 `00001f0` 对应文件偏移 0x1F0，最后两个字节 `aa55` 是引导扇区签名（0xAA55 小端序）

**输出示例（hexdump -C，带 ASCII）：**
```
...
**说明：**
- 左侧是文件偏移量（十六进制）
- 中间是每 16 字节的十六进制内容（每字节单独显示）
- 右侧是 ASCII 字符表示（可打印字符显示，不可打印字符显示为 `.`）
- 可以看到字符串 "Hello from Boot Sector!" 在偏移 0x0010-0x0020 附近
