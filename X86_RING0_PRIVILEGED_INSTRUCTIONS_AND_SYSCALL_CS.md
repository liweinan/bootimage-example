...
## 1. 用户态调用 syscall 时如何切回 kernel CS？

**“只有 Ring 0 才能切换段”** 指的是：在用户态不能执行 LGDT、LTR、或随意用远跳/远调用把 CS 加载成内核段等**特权指令**，否则会 #GP。

但 **syscall / int 0x80** 的段切换**不是用户去执行“加载 CS”的指令**，而是 **CPU 在响应这条指令时，按内核事先配好的 MSR 或 IDT 自动加载新的 CS**。

- **SYSCALL**：用户执行 `syscall`（非特权指令）。CPU 从 **MSR**（如 IA32_STAR）取出内核 CS/SS 选择子，**自动加载 CS**，CPL 变为 0。这些 MSR 是内核在启动时（Ring 0）用 WRMSR 写好的。
- **INT 0x80**：用户执行 `int 0x80`。CPU 查 **IDT** 中 0x80 的门描述符，从门里的段选择子**自动加载 CS**，CPL 变为 0。IDT 和门是内核在 Ring 0 下建好的。

因此：**谁“决定”切到哪个段**是内核（Ring 0）通过 MSR 或 IDT 配置的；**谁“执行”这次 CS 加载**是 CPU 在处理 syscall/int 时的硬件行为。用户只是在 Ring 3 执行了一条受控的入口指令，CPU 按内核预设完成段与特权级切换。
