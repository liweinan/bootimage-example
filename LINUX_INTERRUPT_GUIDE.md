...
> - **术语说明**：本文讨论的是**硬件中断处理**。注意："软中断 softirq"是内核的延迟处理机制，与 `INT n` 指令（Software Interrupt）是不同的概念

---

## IDT 与中断类型概览

**重要概念澄清**：为避免混淆，先明确 IDT 与不同中断类型的关系。

> **详细的基础概念**：关于 Interrupt/Exception/Trap 的 Intel SDM 官方定义、三者本质区别、Exception 分类（Fault/Trap/Abort）、优先级等详细说明，请参见 [X86_INTERRUPT_EXCEPTION_TRAP.md](X86_INTERRUPT_EXCEPTION_TRAP.md)。
...
| **软件中断** | 特定向量 | INT 0x80（32位系统调用兼容） | idt_setup_traps() |

**注意**：虽然习惯上称 `INT 0x80` 为"软件中断"，但在 Intel SDM 和 CPU 硬件层面，它被归类为 **Exception**（同步触发、不受 IF 控制）。详见 [X86_INTERRUPT_EXCEPTION_TRAP.md](X86_INTERRUPT_EXCEPTION_TRAP.md)。
...
> 详细的系统调用机制对比见 [Linux 系统调用初始化详解](LINUX_KERNEL_SYSCALL_INIT.md)。
...
4. **系统调用不是 Top Half**
   - 传统系统调用可通过中断机制实现（如 `INT 0x80`，查询 IDT），但现代 64 位系统主要使用 `SYSCALL` 指令（通过 MSR，不查询 IDT）
   - 无论哪种方式，系统调用都是主动触发的，不是硬件异步触发的，因此不属于 Top Half
...
- **Bottom Half 是“软件中断”吗？**  
  **部分机制是，部分不是。** “Bottom Half”是设计概念（中断返回后的延后处理），具体有三种机制：**Softirq** 和 **Tasklet** 在**软中断上下文**中执行，属于软件中断；**Workqueue** 在**进程上下文**中由内核线程执行，不是软件中断。

- **需要主动调用还是被动执行？**  
  **被动执行。** Top Half 只负责**标记/调度**（如 `raise_softirq()`、`tasklet_schedule()`、`schedule_work()`），不直接调用下半部处理函数。内核在**合适时机**（中断返回后、ksoftirqd、worker 线程等）检查待处理项并执行对应处理函数，驱动不能指定“立刻在这里跑完下半部”。
