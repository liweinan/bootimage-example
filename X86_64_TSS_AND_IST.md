...
11. [为什么需要 IST](#11-为什么需要-ist)（含 [§11.1.1 内核栈场景](#1111-该示例主要对应内核栈场景)、[§11.4 IST 使用后的状态恢复](#114-ist-使用后的状态恢复)）
12. [启动早期约束](#12-启动早期约束)
13. [调试与验证](#13-调试与验证)
...
§11.1 的 Triple Fault 链**主要描述内核态、在线程内核栈上发生的级联故障**，而不是普通用户态程序在用户栈上溢出的典型路径。

**§11.1 的关键**：没有 IST 时，异常帧压在**当前栈**上；若**当前栈已损坏**，#DF 仍会在同一条栈上压帧，才会 Triple Fault。

| 发生位置 | #PF（如栈溢出）时「当前栈」 | #PF 处理程序压栈用哪条栈 |
|----------|----------------------------|--------------------------|
| **内核态**（syscall、中断下半部、内核线程等） | 线程的 **kernel stack** | 仍是这条 **kernel stack**（#PF 的 IDT.IST=0，见 §11.3） |
| **用户态** | 用户栈 | 进异常处理后会切到 **kernel stack**（TSS / entry 机制），**不是**在坏的用户栈上压帧 |

**典型内核栈 Triple Fault 路径**：

1. 某**内核代码路径**（内核线程、syscall 处理、驱动等）在内核栈上递归或大栈帧导致溢出；
2. 碰到 kernel stack 底部的 **Guard Page** 触发 #PF；
3. #PF 处理仍用**同一条已溢出的 kernel stack** 压 `pt_regs`；
4. 再失败 → #DF → 若 #DF 也无 IST，仍在坏栈上 → Triple Fault → 重启且无日志。

这与 §6.6 的表述一致：Guard Page 在**进程内核栈**上，栈溢出先 #PF；只有 Guard 被跳过的极端情况才会 #DF。

**与用户态程序的关系**：

- **用户程序栈溢出**：通常是用户态 #PF → 处理程序在**正常的 kernel stack** 上运行 → 可以 `handle_stack_overflow` / 终止进程，**不一定会**走 §11.1 的 Triple Fault 链。
- **间接相关**：用户代码经 syscall 进入内核后，若在内核栈上把栈用爆，则回到上述内核栈场景。

**#DF 与 fault 来源无关**：§9.1 规定 #DF **不区分**用户/内核来源，一律走 IST——因为触发 #DF 的前提就是「用于压异常帧的栈可能已损坏」。§11.1 的**举例**侧重栈溢出这条路径，故读起来像 kernel stack；IST 对 #DF 的设计动机则是：**无论 fault 从哪来，都换到一条已知良好的独立栈**。

> **一句话**：§11.1 的 Triple Fault 例子说的是**内核在执行、且线程 kernel stack 已损坏/溢出**时的级联；不是指用户态 prog 在用户栈上溢出就直接 Triple Fault。用户栈溢出通常还有一层 kernel stack 做缓冲；真正危险的是**内核栈本身**已不可用。
