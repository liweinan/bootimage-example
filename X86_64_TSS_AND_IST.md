...
### 2.0 前置背景：实模式（无硬件任务切换）

在讨论 TSS 之前，需要先了解 x86 CPU 的实模式。**实模式没有 TSS，也没有硬件任务切换机制**。

**为什么 CPU 需要从硬件层面提供保护机制？**  
DOS 理论上可以通过时钟中断提供抢占式调度，但它无法阻止用户态程序读写任意内存、钩住中断服务，也无法对 TSR 等驻留程序实现有效的内存和运行管理。因此，CPU 需要从硬件层面提供机制，让操作系统能够区分用户态与内核态，并为用户态提供虚拟内存空间，禁止用户态直接访问物理地址。这样，只有内核才有权限做内存管理、中断服务和进程调度，用户态程序无法抢占系统权限。

#### 实模式的特点

| 特性 | 实模式 | 保护模式 |
|------|--------|----------|
| 地址空间 | 1MB（20 位） | 4GB+（32/64 位） |
| 特权级 | 无（相当于 Ring 0） | Ring 0-3 |
| 内存保护 | 无 | 有 |
| TSS | **无** | 有 |
| 硬件任务切换 | **无** | 有（但现代 OS 不用） |

实模式继承自 8086 的简单架构，本质上是为**单任务**设计的。

#### 实模式下如何实现"多任务"？

如果需要在实模式下实现类似多任务的效果，**完全由软件实现**：

**方案 1：协作式多任务（Cooperative）**

```asm
; 任务主动让出 CPU
task1:
    ; 做一些工作
    call yield          ; 主动切换到下一个任务
    jmp task1
...
yield:
    ...
    push di
    push bp
    mov [current_task_sp], sp   ; 保存当前栈指针
    mov [current_task_ss], ss   ; 保存栈段
...
```asm
; 定时器中断处理程序（INT 8 或 INT 1Ch）
timer_isr:
    ; CPU 自动保存 FLAGS, CS, IP 到当前栈
    ...
    mov [task_table + current_task * 4], sp  ; 保存当前任务 SP
    
    ; 选择下一个任务
    inc current_task
    
    ; 恢复下一个任务
    mov sp, [task_table + current_task * 4]
...
task1_tcb:
    .sp     dw 0        ; 保存的 SP
    .ss     dw 0        ; 保存的 SS
    .regs   times 8 dw 0 ; 其他寄存器（AX, BX, CX, DX, SI, DI, BP, FLAGS）

; 保存当前任务状态
save_task:
    mov [task1_tcb.sp], sp
    mov [task1_tcb.ss], ss
    ; ... 保存其他寄存器到 task1_tcb.regs
```

#### DOS 时代的"多任务"

**DOS 本身是单任务操作系统**，但有一些技术实现有限的"多任务"：

| 技术 | 说明 |
|------|------|
| **TSR（Terminate and Stay Resident）** | 程序驻留内存，通过钩住中断实现后台运行 |
| **DESQview** | 完全软件实现的多任务环境 |
| **Windows 3.x** | 在实模式/保护模式间切换，协作式多任务 |

> 📖 **详细说明**：TSR 的工作原理、驻留机制、著名程序、代码示例等内容，请参见 **[DOS TSR 程序详解](DOS_TSR_EXPLAINED.md)**。
...
从设计思路看，**DOS 具备用定时中断做抢占式调度的硬件基础，但有意（或受限于时代）没有实现**。

| 比喻 | 行为 |
|------|------|
| **Linux/Unix** | 像严格老师：运行进程时会看表（定时中断），时间一到就换人执行，是**强制性时间片**。 |
| **DOS** | 像宽松图书管理员：把书交给一个人后等着，只有对方主动还书（调用系统调用）时才交给下一个人；若一直不还，就一直等。 |

**DOS 选择协同式的主要原因**：

1. **单任务设计目标**：DOS 为单用户、单任务设计，目标是把硬件控制权交给当前程序。当时用户通常一次只跑一个任务，没有多任务需求。
2. **简化与性能**：抢占式需要管理进程状态、时间片、同步，增加代码和复杂度；每次定时中断都要做调度和可能的切换，在几 MHz 到几十 MHz 的年代，这种开销难以接受。把 CPU 全给当前程序能让它跑得更快。
3. **应用程序的“信任”**：协同式下，开发者可以假设自己独占 CPU 直到主动交还，编程模型简单；但一个设计不良的程序就会让整个系统卡死。

**若 DOS 强行用定时中断做抢占式调度，会面临**：

1. **内核不可重入**：DOS 核心（文件读写、显示等）**不是为中断设计的**。若程序 A 正在执行 DOS 内部命令（如修改文件系统结构），定时中断发生并切换到程序 B，B 又调用 DOS，可能破坏 A 正在使用的内部数据，导致崩溃。
2. **缺乏进程隔离**：无内存保护时，即使强制切换进程，新进程也可能覆盖旧进程数据，恢复时出错；程序之间可随意互相干扰。
3. **设备冲突**：一个程序正在操作声卡/显卡时被切换走，另一程序再操作同一硬件，硬件状态会错乱。

**小结**：从微观技术上看，DOS 完全可以开定时中断，在中断里保存/恢复状态做抢占；但这违背其设计初衷，并会立刻带来内核不可重入、数据混乱、设备冲突等问题。**DOS 没有实现抢占式多任务，不是因为做不到，而是因为它面向不同时代与需求，选择了简单、高效、信任应用的协同式道路**；现代 OS 则选择复杂、安全、强制性的抢占式道路，由内核承担管理与保护责任。

真正的硬件辅助任务切换要等到 80286 引入保护模式才出现，而 TSS 正是保护模式的核心组件之一。
...
在讨论栈切换时，经常会涉及到 SP（栈指针）和 SS（栈段）两个寄存器。这里详细解释它们的区别和用法。

#### 15.6.1 基本概念

| 寄存器 | 全称 | 位数 | 作用 | 是否必须 |
|--------|------|------|------|----------|
| **SP/ESP/RSP** | Stack Pointer | 16/32/64位 | **栈顶指针**，指向当前栈顶的偏移地址 | **必须** |
| **SS** | Stack Segment | 16位 | **栈段选择子**，指向描述栈的段描述符 | x86架构需要，x86-64简化 |

#### 15.6.2 在实模式/保护模式下的关系
...
_start:
    ; 设置SS和SP
    mov ax, 0x10        ; 假设0x10是数据段/栈段选择子
    mov ss, ax          ; 设置栈段
    mov esp, 0x7FFF     ; 设置栈顶偏移
    
    ; 现在push/pop使用SS:ESP
    push eax            ; 实际地址 = SS基址 + ESP
    push ebx            ; ESP自动减4
    push ecx
    ...
    ; 直接访问栈内存
    mov eax, [ss:esp+8] ; 明确指定段前缀（通常可省略）
```
...
_start:
    ...
    ; 栈操作只使用RSP
    mov rsp, 0x7FFFFFFF  ; 直接设置64位栈指针
    push rax              ; 实际地址 = 0 + RSP
    push rbx
    push rcx
    
    ; 可以直接用RSP访问
    mov rax, [rsp+16]     ; 访问栈上的数据，不需要SS前缀
```
...
| **类型** | 通用寄存器 | 段寄存器 |
| **内容** | 偏移地址/线性地址 | 段选择子 |
| **自动变化** | push/pop/call/ret等自动增减 | 不变，除非显式加载 |
| **特权级切换** | CPU从TSS加载新值 | CPU从TSS加载新值 |
| **64位重要性** | 极高，完全控制栈 | 较低，仅用于特权级标识 |
| **用户程序可修改** | 可以（谨慎） | 通常不能（需要特权级0） |
| **典型值** | 任何有效地址 | 0x0010（内核）或0x001B（用户） |

**硬件层面**：
- **SS**：段寄存器，存储段选择子，CPU用它来访问段描述符，获取段属性（基址、权限、界限）
- **SP**：通用寄存器，专门用于栈操作时自动增减

**权限控制**：
- **SS**：包含请求特权级（RPL），CPU检查当前栈操作是否合法
- **SP**：只是数值，不包含权限信息

#### 15.6.5 特权级切换时的变化

当从用户态（Ring 3）切换到内核态（Ring 0）时：

```asm
; 用户态执行：
; SS = 0x001B (用户数据段，RPL=3)
; RSP = 0x7FFFFFF0 (用户栈顶)

; 发生中断/异常：
; 1. CPU从TSS中读取Ring 0的SS0和RSP0
; 2. CPU自动切换：
;    SS = 0x0010 (内核数据段，RPL=0)
;    RSP = 0xFFFF800000001000 (内核栈顶)
; 3. 自动压栈保存用户态的SS和RSP
...
```asm
; 函数调用过程：
call func   ; 自动 push RIP (使用SS:RSP)
push rbp    ; 手动 push (使用SS:RSP)
; ...函数执行...
pop rbp     ; 手动 pop (使用SS:RSP)
ret         ; 自动 pop RIP (使用SS:RSP)
```
...
__switch_to:
    ...
    ; 切换到新进程的栈
    mov rsp, [rdi + TASK_stack]  ; 新进程的内核栈指针
...
; 1. 特权级标识（通过RPL位）
; 2. 兼容32位代码
; 3. 某些特殊的系统指令（如syscall/sysret）

; 用户态SS值（典型）：
; RPL=3, TI=0, Index=某个值
; 表示：特权级3的段，在GDT中

; 内核态SS值（典型）：
; RPL=0, TI=0, Index=某个值
; 表示：特权级0的段，在GDT中
```
...
| **本质** | 栈顶位置 | 栈所在的段 |
| **现代64位** | 核心作用 | 辅助作用 |
| **隐喻** | "我在栈的哪个位置" | "我在哪个段里运行" |

简单来说：**SS告诉CPU"我在哪个段里运行"，SP告诉CPU"我现在在栈的哪个位置"**。在现代64位系统中，SS的角色被大大简化（因为段基址强制为0），而SP则是栈操作的核心。
