...
| **MSR（STAR/LSTAR/FMASK、`syscall_init`）** | [LINUX_X86_MSR_REFERENCE.md](LINUX_X86_MSR_REFERENCE.md) | ⭐⭐⭐⭐ 核心 |
...
5. X86_64_TSS_AND_IST.md（TSS 与 IST 机制）
   ...
   └─ TSS 初始化时机
   ↓
6. LINUX_X86_MSR_REFERENCE.md（MSR 与快速系统调用配置）→ LINUX_KERNEL_SYSCALL_INIT.md（系统调用详解）
   ├─ STAR/LSTAR/FMASK、SYSENTER MSRs
   ├─ INT 0x80 机制
   ├─ SYSCALL/SYSENTER 机制
   └─ 性能对比
   ↓
...
| **SeaBIOS & GRUB 是否遵守 ABI？** | [SEABIOS_GRUB_ABI_COMPLIANCE_ANALYSIS.md](SEABIOS_GRUB_ABI_COMPLIANCE_ANALYSIS.md) ⭐ 新增 | ⭐⭐⭐⭐ 核心 |
...
2. 深入理解 Linux 内核 ABI（LINUX_KERNEL_FUNCTION_ATTRIBUTES.md 1.7节）⭐ 新增
   ...
   ↓
3. **分析 Linux 内核启动代码 ABI 遵守情况**（LINUX_KERNEL_ABI_COMPLIANCE_ANALYSIS.md）⭐ 新增
   ...
   ↓
3.5. **分析 SeaBIOS & GRUB ABI 遵守情况**（SEABIOS_GRUB_ABI_COMPLIANCE_ANALYSIS.md）⭐ 新增
   ├─ SeaBIOS i386 平台（合规性 30%，-mregparm=3）
   ├─ GRUB i386 平台（合规性 25%，-mregparm=3 + -mrtd）
   ├─ GRUB x86-64 平台（合规性 95%，正确使用 -mno-red-zone）
   ├─ GRUB ARM32 平台（合规性 98%，遵循 AAPCS）
   ├─ GRUB ARM64 平台（合规性 100%，遵循 AAPCS64）
   ├─ 为何偏离标准（代码体积节省 10-15%、性能优化）
   └─ 与 Linux 内核的异同对比
   ↓
...
| **ABI（应用程序二进制接口）** | [LINUX_KERNEL_ABI_COMPLIANCE_ANALYSIS.md](LINUX_KERNEL_ABI_COMPLIANCE_ANALYSIS.md) ⭐ 新增, [SEABIOS_GRUB_ABI_COMPLIANCE_ANALYSIS.md](SEABIOS_GRUB_ABI_COMPLIANCE_ANALYSIS.md) ⭐ 新增, [LINUX_KERNEL_FUNCTION_ATTRIBUTES.md](LINUX_KERNEL_FUNCTION_ATTRIBUTES.md) |
...
| 主题 | 相关文档 |
|------|---------|
| **SeaBIOS** | [SEABIOS_LOAD_BOOT_SECTOR.md](SEABIOS_LOAD_BOOT_SECTOR.md), [SEABIOS_E820_CONSTRUCTION.md](SEABIOS_E820_CONSTRUCTION.md), [SEABIOS_INT09_KEYBOARD_HANDLER.md](SEABIOS_INT09_KEYBOARD_HANDLER.md) ⭐ 新增 |
| **Slab 分配器** | [SLAB_ALLOCATOR_EXPLAINED.md](SLAB_ALLOCATOR_EXPLAINED.md), [BUDDY_ALLOCATOR_GUIDE.md](BUDDY_ALLOCATOR_GUIDE.md) |
| **系统调用** | [LINUX_X86_MSR_REFERENCE.md](LINUX_X86_MSR_REFERENCE.md)（MSR）、[LINUX_KERNEL_SYSCALL_INIT.md](LINUX_KERNEL_SYSCALL_INIT.md)（流程与汇编） |
| **SYSCALL 指令** | [LINUX_X86_MSR_REFERENCE.md](LINUX_X86_MSR_REFERENCE.md)、[LINUX_KERNEL_SYSCALL_INIT.md](LINUX_KERNEL_SYSCALL_INIT.md) |
| **TLB（地址转换缓冲）** | [X86_64_TLB_MANAGEMENT.md](X86_64_TLB_MANAGEMENT.md) |
| **TSR（终止并驻留）** | [DOS_TSR_EXPLAINED.md](DOS_TSR_EXPLAINED.md) ⭐ 新增 |
...
**最新更新**：
- ⭐ 新增 [SEABIOS_INT09_KEYBOARD_HANDLER.md](SEABIOS_INT09_KEYBOARD_HANDLER.md) - SeaBIOS INT 09h 键盘中断处理程序分析（汇编入口、寄存器保存、扫描码处理）
- ⭐ 新增 [DOS_TSR_EXPLAINED.md](DOS_TSR_EXPLAINED.md) - DOS TSR 程序详解（驻留机制、中断钩子、著名程序、演示代码）
- ⭐ 新增 [SEABIOS_GRUB_ABI_COMPLIANCE_ANALYSIS.md](SEABIOS_GRUB_ABI_COMPLIANCE_ANALYSIS.md) - SeaBIOS & GRUB ABI 遵从性分析（i386/x86-64/ARM32/ARM64 全平台）
