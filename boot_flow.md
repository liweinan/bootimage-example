...
2. [BIOS 运行模式与内存访问详解](#2-bios-运行模式与内存访问详解)
3. [SeaBIOS 初始化中断服务](#3-seabios-初始化中断服务)
4. [引导扇区程序：从 SeaBIOS 到用户代码的执行](#4-引导扇区程序从-seabios-到用户代码的执行)
5. [boot.asm 与 GRUB boot.S 的相似之处](#5-bootasm-与-grub-boots-的相似之处)
6. [Linux 内核接管 BIOS](#6-linux-内核接管-bios)
7. [总结：完整流程时间线](#总结完整流程时间线)
8. [关键源代码文件索引](#关键源代码文件索引)
9. [附录A：键盘中断处理代码分析](APPENDIX_A_KEYBOARD_INTERRUPT.md)
10. [附录B：应用层事件机制与操作系统实现](APPENDIX_B_EVENT_MECHANISM.md)
11. [中断处理详解](INTERRUPT_HANDLING.md)
...
### 2.1 实模式（Real Mode）与保护模式（Protected Mode）详解
...
#### 2.2.1 为什么BIOS映射到实模式内存空间只有128KB，其他的部分如何访问执行？
...
#### 2.2.3 问题 2：为什么 BIOS 存储在 4GB 地址空间顶部？
...
#### 2.2.4 问题 2.1：在更大内存的主机上，4GB 地址空间计算还适用吗？
...
#### 2.2.5 问题 2.2：QEMU 和 SeaBIOS 如何支持更大内存的虚拟机？
...
#### 2.2.6 问题 2.3：64 位虚拟机如何支持 32 位内存地址？
...
#### 2.2.7 问题 2.4：实际硬件（64位CPU）如何支持32位内存地址？
...
#### 2.2.8 问题 3：地址 `0x100000000ULL - bios_size` 是实模式吗？
...
### 6.1 GRUB 如何加载内核到 head_64.S 入口点
...
### 6.5 BIOS IVT 与 Kernel IDT 的软件中断服务程序对比
